...
outf1 = open(os.path.join(fol_path,'Lib1000sweep_T.txt'),'w')
outf2 = open(os.path.join(fol_path,'Lib1000sweep_E.txt'),'w')
PC_period = 0.562
h_alumina = 0.06
wavelength=1.55  
t_start = time.time() 
...
for h in range (500,1500,10):
    ...
    tmid1 = time.time()
    for hole_radius_nm in range(100, 450, 2):
    ...
    # Slab mode effective index=2.846334 for 40nm Al2O3, theta=37.83
        S.SetExcitationPlanewave(
            IncidenceAngles=(37.83,0),# (polar in [0,180],azimuthal in [0,360)
            sAmplitude=1,
            pAmplitude=0,
            Order=0)        
